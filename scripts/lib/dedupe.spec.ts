...
describe('dedupeByKeyKeepingLatest', () => {
	it('keeps a single item unchanged', () => {
		...
		expect(dedupeByKeyKeepingLatest(items, (i) => i.slug, (i) => i.pubDate)).toEqual(items);
	});
	...
	it('drops the older duplicate, keeping the newer occurrence by date', () => {
		...
		expect(dedupeByKeyKeepingLatest([older, newer], (i) => i.slug, (i) => i.pubDate)).toEqual([
			newer
		]);
		expect(dedupeByKeyKeepingLatest([newer, older], (i) => i.slug, (i) => i.pubDate)).toEqual([
			newer
		]);
	});
