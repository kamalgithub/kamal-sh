...
<ul class="grid grid-cols-[repeat(auto-fit,minmax(220px,1fr))] gap-x-6 gap-y-8">
	{#each videos as video (video.videoId)}
		<li>
			...
			<a href={video.link} target="_blank" rel="noreferrer" class="group block">
				<img
					src={video.thumbnail}
					alt=""
					loading="lazy"
				...
				/>
				<p
					class="mt-3 text-small font-medium text-text transition-colors duration-(--duration-fast) ease-standard group-hover:text-accent"
				>
					{video.title}
				</p>
