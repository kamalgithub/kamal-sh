...
describe('getTimeSlotsForDate', () => {
	it('generates slots spanning the 11am-2pm IST window at the given duration', () => {
		const slots = getTimeSlotsForDate(date, 30, FAR_PAST);

		expect(slots).toHaveLength(6); // 3 hours / 30 min
		expect(slots[0].toISOString()).toBe('2026-09-05T05:30:00.000Z'); // 11:00 AM IST
		expect(slots.at(-1)?.toISOString()).toBe('2026-09-05T08:00:00.000Z'); // 1:30 PM IST, last 30-min slot
	});

	it('produces fewer, evenly spaced slots for a longer duration', () => {
		const slots = getTimeSlotsForDate(date, 60, FAR_PAST);

		expect(slots).toHaveLength(3); // 3 hours / 60 min
		expect(slots[0].toISOString()).toBe('2026-09-05T05:30:00.000Z');
		expect(slots.at(-1)?.toISOString()).toBe('2026-09-05T07:30:00.000Z'); // 1:00 PM IST
	});
