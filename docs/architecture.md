...
- **SSR only where genuinely required.** A handful of routes opt out with `export const prerender = false`, each for a specific per-request need:
  ...
  - `/newsletter` — the writing-updates subscribe form's action (Mailgun mailing list). `/writing`'s embedded copy of the same form posts here via `action="/newsletter"`, so `/writing` itself stays fully prerendered — only this one small route needs a server round trip.
