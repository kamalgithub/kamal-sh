...
11. **Every outbound fetch sets a timeout.** `signal: AbortSignal.timeout(...)` on every server-side call to a third party (Mailjet, Turnstile, Cloudflare's API, GitHub) — a hung upstream must never stall a request indefinitely. See [docs/conventions.md](./docs/conventions.md)'s "Outbound fetch timeouts."
